...
find .  -maxdepth "$Depth"  -type f -exec bash -c 'grep -Iq . "{}"' \; -and -print0 | xargs -0 -n 1   bash -c 'file --mime "$@" | grep -v  "charset=utf-8\|ascii"| cut -d : -f 1 | grep -v  ".txt\|.bat\|.sh\|.eps" ;' bash | xargs -d '\n' -n 1  bash -c  'iconv -f GBK -t UTF-8  "$@" -o "$@"; echo "Converting file$@ .........................Finished";' bash
